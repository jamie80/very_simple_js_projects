...
const fieldsForValidation = [...form.elements].filter(
	(element) => element.type !== "submit"
);

const installmentPeriod = document.getElementById("installmentsPeriod");
...
form.addEventListener("submit", (e) => {
	...
	if (form.checkValidity()) {
		...
		fieldsForValidation.forEach((field) => setFieldErrorHintVisibility(field));
	}
});

fieldsForValidation.forEach((field) => {
	field.addEventListener("input", (e) => setFieldErrorHintVisibility(e.target));
});
...
function sendCreditForm() {
	axios
		.post("/save-credit-form", getFormData())
		.then((response) => {
